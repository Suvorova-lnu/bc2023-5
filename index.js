...
//GET home
app.get("/", (req, res) => {
    res.send(" /notes, /UploadForm.html, /upload, /notes/:name")
})
...
app.post("/upload", (req, res) => {
    ...
    if (notes.some((note) => note.note_name === noteName)) {
        res.status(400).send('Sorry, note with that name already exists!');
    } else {
        const newNote = { note_name: noteName, note: noteContent };
        notes.push(newNote);
        fs.writeFileSync(notesList, JSON.stringify(notes, null, 2), type);
        res.status(201).send('Greetings! Note has been created.');
    }
});
...
app.get("/notes/:name", (req, res) => {
    ...
    if(note) {
        ...
        res.status(404).send("Sorry, such note is not found. Please, try another name.");
    }
});
...
app.put("/notes/:name", (req, res) => {
    ...
    if(note) {
        ...
        fs.writeFileSync(notesList, JSON.stringify(notes, null, 2), type);
        res.status(200).send('Congratulations! Note has been successfully updated.');
    } else {
        res.status(404).send("Sorry such note is not found.Please try another name.");
    }
});
...
app.delete("/notes/:name", (req, res) => {
    ...
    if (idx !== -1) {
        ...
        res.status(404).send("Sorry such note is not found.npm");
    }
});
